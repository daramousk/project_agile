...
# I am aware that the following code is not extendable but Odoo started first ;)
...
def relaxng(view_type):
    ...
    if view_type not in view_validation._relaxng_cache:
        with tools.file_open(os.path.join(
                view_type == 'diagram' and 'web_diagram_position' or 'base', 'rng', '%s_view.rng' % view_type)
        ) as frng:
            try:
