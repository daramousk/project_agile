...
class Sprint(models.Model):
    ...
    name = fields.Char(
        default=lambda self: self.env['ir.sequence'].next_by_code('project_agile.sprint.sequence'),
        agile=True
    )
    ...
    state = fields.Selection(
        selection=[('draft', 'Draft'), ('active', 'Active'), ('completed', 'Completed')],
        required=True,
        default='draft',
        agile=True
    )
    ...
    default_hrs = fields.Float(related='team_id.default_hrs', string='Default daily hours')

    @api.multi
    @api.depends('task_ids', 'task_ids.story_points')
    def _compute_total_story_points(self):
        for record in self:
            record.total_story_points = sum(int(t.story_points) for t in record.task_ids)
    ...
    def _compute_velocity(self):
        for sprint in self:
            sprint.velocity = sum(
                int(t.story_points) for t in sprint.task_ids.filtered(lambda x: x.wkf_state_type == "done")
            )
    ...
    def _compute_length_days(self):
        if self.start_date and self.end_date:
            self.sprint_length_days = (fields.Date.from_string(self.end_date) -
                                       fields.Date.from_string(self.start_date)).days
        else:
    ...
    def create(self, vals):
        if not self.env.user.team_id:
            raise exceptions.ValidationError(_("You have to be part of an agile team in order to create a new sprint"))

        if 'name' not in vals:
            vals['name'] = "Sprint %s" % self.env.user.team_id.sprint_sequence

        self.env.user.team_id.sprint_sequence = self.env.user.team_id.sprint_sequence + 1
    ...
    def open_sprint_in_odoo_agile(self):
        ...
        if self.state == 'completed':
            raise exceptions.ValidationError(_("Only future or active sprint can be opened in Odoo Agile!"))
